          ...
          <tbody className="divide-y divide-gray-100">
            {filteredAndSortedUsers.map((user) => (
              <tr key={user.id} className={`hover:bg-gray-50 transition-colors group ${user.isWithdrawn ? 'opacity-60 bg-gray-50/30' : ''}`}>
                ...
                <td className="px-4 py-3">
                  {!user.isWithdrawn ? (
                    <div className="relative inline-block text-left group/role">
                      <button className={`inline-flex items-center px-2 py-1 rounded-none text-[10px] font-black transition-colors ${
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-none text-[10px] font-black ${
                    user.status === '영구정지' ? 'bg-black text-white' :
                    ...
                    <div className="flex items-center justify-end space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button 
                        ...
                        className={`p-1.5 rounded-none transition-colors ${
                          user.isSuspended ? 'hover:bg-green-100 text-green-600' : 'hover:bg-red-100 text-red-600'
                        }`}
