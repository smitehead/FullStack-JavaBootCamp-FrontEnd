  ...
  const handleSave = () => {
    if (editingNotice) {
      setNotices(prev => prev.map(n => n.id === editingNotice.id ? {
        ...n, title, category, description: '', content, isImportant
      } : n));
    } else {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-gray-900 tracking-tight">공지사항 관리</h1>
          <p className="text-gray-500 mt-1 text-[11px] font-medium">서비스 공지사항을 등록하고 관리합니다.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredNotices.map((notice) => (
          <div key={notice.id} className="bg-white p-5 rounded-none shadow-sm border border-gray-100 flex items-start justify-between group hover:shadow-md transition-all">
            <div className="flex-1">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-[10px] font-medium text-gray-400">{notice.createdAt.split('T')[0]}</span>
              </div>
              <h3 className="text-base font-black text-gray-900 mb-1">{notice.title}</h3>
            </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-none p-8 shadow-2xl animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 mb-8">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black text-gray-700 mb-2">카테고리</label>
                  <select 
                ...
                <div className="flex items-end pb-3">
                  <label className="flex items-center space-x-2 cursor-pointer group">
                    ...
                    />
                    <span className="text-sm font-black text-gray-700 group-hover:text-[#FF5A5A] transition-colors">중요 공지로 설정</span>
                  </label>
                </div>
              </div>

              <div>
                <label className="block text-xs font-black text-gray-700 mb-2">제목</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-xs font-black text-gray-700 mb-2">상세 내용</label>
                <textarea 
                  rows={6}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-none p-8 shadow-2xl animate-in fade-in zoom-in duration-300">
            ...
            <p className="text-gray-500 font-medium text-center mb-8 text-sm">
              삭제된 공지사항은 복구할 수 없습니다.<br />신중하게 결정해주세요.
            </p>
